...
This shows that if an AMD module loader is used then the library is declared using the `define()` function. Otherwise, the library is declared as a single class in the global namespace. Both approaches encapsulate chart functions and prevents the global namespace from getting polluted.
...
The algorithm is passed all possible values for the current article and data source, and the fractional year as arguments. Data values are held in an multidimensional array, as shown in the following example.
